...
import com.orchestration.federation.FederationData.*
  ...
  val standardWrappers: Wrapper[Any] =
    maxFields(200) |+| // query analyzer that limit query fields
      ...
      ApolloFederatedTracing
        .wrapper() // wrapper for https://github.com/apollographql/apollo-tracing

  object Characters extends GenericSchema[CharacterService] {
    import com.orchestration.federation.FederationData.characters._
    import auto._
    ...
    case class Mutations(
        deleteCharacter: CharacterArgs => URIO[CharacterService, Boolean]
    )
    case class Subscriptions(
        characterDeleted: ZStream[CharacterService, Nothing, String]
    )
    ...
    implicit val characterSchema: Schema[CharacterService, Character] =
      ...
      obj("Character", None, directives = List(GQLKey("name").directive))(
        implicit ft =>
          List(
            field("name")(_.name),
    ...
    val withFederation: GraphQLAspect[Nothing, CharacterService] =
      federated(
        EntityResolver.from[CharacterArgs](args =>
          ZQuery.fromZIO(CharacterService.findCharacter(args.name))
        ),
        EntityResolver.from[EpisodeArgs](args =>
          ZQuery
            .fromZIO(
              CharacterService.getCharactersByEpisode(args.season, args.episode)
            )
            .map(characters =>
              Some(
                Episode(
    ...
    val api: GraphQL[CharacterService] =
      graphQL[CharacterService, Queries, Mutations, Unit](
        ...
        // ???
        // implicit val charactersArgSchema: Schema[Any, CharactersArgs] = Schema.gen
      ) @@ standardWrappers @@ withFederation
  }

  object Episodes extends GenericSchema[EpisodeService] {
    import com.orchestration.federation.FederationData.episodes.{
      Episode,
      EpisodeArgs,
      EpisodesArgs
    }
    ...
    val api: GraphQL[EpisodeService] =
      graphQL[EpisodeService, Queries, Unit, Unit](
        ...
        EntityResolver.from[EpisodeArgs](args =>
          ZQuery.fromZIO(EpisodeService.getEpisode(args.season, args.episode))
        )
      )
